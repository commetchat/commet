...
class NotifyingListCombiner<T> implements INotifyingList<T> {
  late NotifyingList<T> _internalList;
  late List<INotifyingList<T>> _sourceLists;

  List<StreamSubscription> subs = List.empty(growable: true);

  NotifyingListCombiner(List<INotifyingList<T>> baseLists) {
    this._internalList =
        NotifyingList.from([for (var i in baseLists) ...i], growable: true);

    for (var item in baseLists) {
      subs.addAll([
        item.onRemove.listen(_onBaseItemRemoved),
        item.onAdd.listen(_onBaseItemAdded),
      ]);
    }

    _sourceLists = baseLists;
  }

  @override
  void unsubscribe() {
    for (var list in _sourceLists) {
      list.unsubscribe();
    }
